...
<Note>
  `awaiting_approval` is also used for the [large-edit continue prompt](/guides/async-jobs#continuing-a-large-edit). Tell them apart with `metadata.awaiting_kind`: when it is `"continue_prompt"` there are no `pending_changes` — resume that with `/continue`, not `/approve`.
</Note>
...
// 3. Collect proposed changes. Multi-change turns arrive as a single
//    `proposed_change_batch` event; isolated single-change turns arrive
//    as a `proposed_change` event. Register both listeners.
let pendingChanges = [];
...
<Note>
After calling `/approve`, the AI resumes processing. If approved changes were applied, the SSE connection will eventually emit a `final` event with the updated document. If you denied with feedback, you may receive new `proposed_change` or `proposed_change_batch` events as the AI tries again.
...
<Warning>
  **Listen for both event names.** Multi-change turns are delivered as a single `proposed_change_batch` event; isolated single-change turns may still arrive as `proposed_change`. If you only subscribe to `proposed_change`, you will silently miss every batched HITL turn — the most common shape for edits across many sections.
</Warning>
