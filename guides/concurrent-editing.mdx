...
Send the editor's current `html` (same body shape as `/save`). It un-archives the document, re-links it to this session, and saves the latest content — idempotent if another session already restored it. REST-only and non-billable.
