...
**Apply per document, not by reloading (`revert_changes`).** The revert response carries `revert_changes` — a map of document id → the change-set for that document. Apply each document's change-set the same way you apply a normal AI result: per section, onto the live editor (see [Applying AI updates safely](/guides/editor-integration#applying-ai-updates-safely-without-losing-user-edits)). This matters in a multi-document session: a single revert can touch several open documents, and a user may be mid-edit in one of them. Applying `revert_changes` section-by-section preserves their in-progress typing and surfaces a keep-both choice on a clashed section, exactly like a live edit. Whole-document `setContent` per tab would throw that away. `document_state` + `editor_action` are still returned for the focused document as a fallback for simple single-document UIs.
