...
1. **Define the SuperDocs tool** in your agent's tool registry. The two required parameters are `message` (the natural-language instruction for the AI) and `session_id` (a string that ties multiple turns together). `document_html` (the HTML the agent wants edited) is optional — send it on the first turn, then omit it: the server persists document state by `session_id` across turns, so resending the full HTML every turn just wastes tokens.
    ...
    "function": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "document_html": {"type": "string", "description": "Current HTML to edit. Omit once the session already holds the document — state persists by session_id."},
...
edit_document_tool = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "document_html": {"type": "string", "description": "Omit once the session already holds the document — state persists by session_id."},
            "approval_mode": {"type": "string", "enum": ["approve_all", "ask_every_time"]},
        },
        "required": ["message", "session_id"],
    },
...
for batch in stream_proposed_change_batches(job_id, session_id):
    ...
    httpx.post(
        ...
        json={"job_id": job_id, "changes": decisions},
    )
```
...
If your agent decides — mid-workflow — that a previous edit was wrong, it can rewind the session to before the user message that triggered the bad edit instead of trying to patch the result forward. Call `POST /v1/sessions/{session_id}/revert` with the `turn_index` of the user message you want to undo. SuperDocs returns the document state from before that turn plus the original message text, the agent can rewrite the instruction more clearly, and the session continues from the rewind point. The original conversation is preserved for audit but hidden from active reads.

Concrete shape — register revert as a second tool alongside `edit_document`:
...
The agent reasons "this edit went wrong, let me try a different prompt" and chains `revert_session` → `edit_document` with the corrected instruction. Returns `409` if a chat job is still in flight on the session — wait for it to settle first.

See the full revert contract in [Sessions → Revert a session to a previous message](/concepts/sessions#revert-a-session-to-a-previous-message).
...
- [SSE Streaming](/guides/streaming) — if you choose `ask_every_time` and need to consume the turn's `proposed_change_batch` event.
