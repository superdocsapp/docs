...
Deep reasoning roughly **6× the output tokens** versus Balanced for a typical 1,000-token edit. On `core` (`gemini-2.5-flash`) that translates to ~$0.04 vs ~$0.007 per edit. On `max` (`gemini-3-flash-preview`) it's higher — ~$0.05 vs ~$0.009. The wall-clock latency increase is typically 1–3 seconds.
