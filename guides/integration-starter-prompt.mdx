...
- `GET v1/chat/{session_id}/stream?job_id=...&api_key=...` — SSE stream of seven event types (`document_sync`, `intermediate`, `proposed_change`, `final`, `usage`, `ui_pointer`, `error`). Browser-side requires the `api_key` query param because `EventSource` can't set custom headers.
