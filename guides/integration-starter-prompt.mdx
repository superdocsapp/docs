...
- `GET v1/chat/{session_id}/stream?job_id=...&api_key=...&last_sequence=...` — SSE stream of eleven event types (`document_sync`, `intermediate`, `proposed_change`, `proposed_change_batch`, `continue_prompt`, `documents_changed`, `model_fallback`, `final`, `usage`, `ui_pointer`, `error`). Browser-side requires the `api_key` query param because `EventSource` can't set custom headers; on reconnect pass `last_sequence` to replay only newer events.
...
- [SSE Streaming](/guides/streaming) — seven event types, the `proposed_change.content` double-parse, and the `EventSource` + `api_key` query-param pattern.
