...
**Target fill time: under 5 minutes.** Describe what you want SuperDocs to do in your product (1-3 sentences), optionally paste your existing agent context (`CLAUDE.md`, `AGENTS.md`, `.cursorrules`) or a `package.json`, and the agent figures out the rest. It will tell you what it inferred before it starts writing code, and you'll have one chance to correct it.
...
Hit the copy button at the top-right of the code block, then paste into your coding agent (Claude Code, Cursor, Windsurf, Codex, Cline, or any other).
...
## Part 0 (REQUIRED) — How do you want SuperDocs in your product?

In 1-3 sentences, describe what you want SuperDocs to do in your product. The coding agent uses this to determine which integration shape applies. **This is the only required field in the entire prompt** — every other section is optional.
...
```
[describe your situation here, 1-3 sentences]
```
...
1. **READ CONTEXT.** Before doing anything else:
   ...
   d. Read the SuperDocs API reference at `https://docs.superdocs.app/llms-full.txt`. Fetch it locally if you don't already have it.
...
6. **WAIT.** Do not start writing code until the integrator has confirmed the inferred values and answered the `UNSET` questions.
