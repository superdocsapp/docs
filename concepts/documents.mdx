...
- **Mode C** — upload the HTML to a pre-signed URL first, then send `upload_id` (use for documents between 20 MB and 100 MB — see [Large documents](#large-documents) below)
...
|-----|---|
| Up to 20 MB | Direct POST to `/v1/documents/export` with inline `html` (Mode A) or a `session_id` (Mode B). |
| 20 MB – 100 MB | Request a pre-signed upload URL via `POST /v1/uploads` (`purpose: "export-html"`), PUT the HTML to it, then call `/v1/documents/export` with `upload_id` (Mode C). |
| Over 100 MB | Use the [email-fallback endpoint](#email-fallback-for-very-large-documents). |

Keep inline POSTs at or under 20 MB. Above that, the hosted transport layer rejects the request (a hard ~32 MB clamp sits in front of the renderer), so switch to the upload-then-export pattern (Mode C) rather than pushing a bigger body through directly.
