...
|------|-----------|
| PDF | `.pdf` |
| Word | `.docx` (`.doc` rolling out) |
| OpenDocument | `.odt` (rolling out) |
| HTML | `.html`, `.htm` |
| Plain text | `.txt` |
| Rich text | `.rtf` |
| Markdown | `.md` |
...
Rich Text (`.rtf`) files are accepted and import as editable text today, with full styling normalization rolling out. Legacy Word (`.doc`) and OpenDocument (`.odt`) conversion onto the same rich pipeline as `.docx` is rolling out; until then those uploads return a clear, actionable error rather than a degraded result.
