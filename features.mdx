...
|---|---|
| `.docx`, `.rtf`, PDF, HTML, Markdown, plain text (`.doc` and `.odt` rolling out) | `.docx`, PDF, HTML, Markdown, plain text |

Same parsing pipeline regardless of input — once a document is loaded into a session, every editing tool works identically. Rich Text (`.rtf`) files are accepted and import as editable text today, with full styling normalization rolling out; legacy Word (`.doc`) and OpenDocument (`.odt`) conversion is rolling out on the same pipeline. Export from any session in any of the five output formats, with per-export customisation for paper size, orientation, margins, filename, image embedding, and optional PDF watermarks. Documents above 20 MB use a pre-signed upload pattern to bypass standard request body limits; documents above 100 MB are rendered asynchronously and delivered via email.
...
PDFs with harder layouts (multiple text columns, dense ruled tables) are read with their reading order and table structure preserved rather than collapsed into a jumble — headings, links, tables, and images come through as structured, editable content, and hyphenated line breaks are rejoined automatically. Scanned or image-only PDFs — where the pages are pictures with no selectable text — are detected and flagged clearly; full text recognition for scanned documents is rolling out.

**Best for:** Working with academic papers and reports in multi-column layouts, extracting tables from data-heavy PDFs.
...
### MCP server — 37 tools, every major client

The same capabilities are exposed as a Model Context Protocol server at `https://api.superdocs.app/mcp/` (Streamable HTTP transport). Compatible clients render the 37 tools natively:
...
All three reach the same 37 MCP tools and the same REST API with identical scoping. Users own and manage their own keys; orgs manage theirs separately.
