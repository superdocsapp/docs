...
Long-running edits and HITL workflows run asynchronously — the API returns a `job_id`, the client polls or subscribes to SSE updates. State persists in a `jobs` table so any backend instance can pick up a job mid-flight (horizontal scaling and redeploys don't drop in-flight work). Approved changes resume automatically.
