...
|------|---------|---------------|
| **400** | Bad Request | Empty message, invalid parameters; a document save that contains no content while the document still has content is refused and the document is left unchanged (the `detail` says so — fix the payload rather than retrying, since an unchanged retry is refused again) |
