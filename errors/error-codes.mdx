...
```json
{"detail": "The AI agent took longer than 10 minutes to process your request. For long edits or multi-step tasks, use chat_async — it streams progress, supports human-in-the-loop approval, and lets you split the work across multiple turns. Try chat_async with a smaller scope per turn (e.g. one section at a time)."}
```
