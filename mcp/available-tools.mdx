...
description: "All 38 MCP tools available when connecting to the SuperDocs MCP server."
---
...
| `get_job` | Get the status, partial results, and any pending changes for an async chat job. Poll after `chat_async`. Pass `compact=true` to omit the heavy result body and streamed intermediate events (status/progress/ids/timestamps/pending-change ids remain) — poll compact while the job runs, then fetch once non-compact when status is `completed` to retrieve the final HTML. | `job_id`, `compact` |
