...
The fastest way to connect SuperDocs to Claude Code is the official plugin — it bundles the MCP server (37 tools), the 4 workflow prompts, and the auto-loading skill in one install. Manual MCP config also works as a fallback.
...
That installs only the MCP server (37 tools). To get the 4 prompts and the skill, you'd need to set those up separately — the plugin install above does all three in one shot.
