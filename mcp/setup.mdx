...
The Model Context Protocol (MCP) lets AI tools connect to external services. SuperDocs exposes an MCP server with 25 tools and 4 user-invocable workflow prompts for document editing, session management, attachments, revert, and more — all on a single endpoint at `https://api.superdocs.app/mcp`.
