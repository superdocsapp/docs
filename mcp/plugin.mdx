...
description: "The official Claude Code plugin — single install bundles 21 MCP tools, 5 workflow prompts, and the auto-loading skill."
---
...
|---|---|---|
| **MCP tools** | 21 | Chat, structural editing, attachments, sessions, jobs, templates, pre-signed upload/download |
| **Workflow prompts** | 5 | `/superdocs:draft_from_outline`, `/superdocs:edit_styled_docx`, `/superdocs:convert_format`, `/superdocs:draft_nda`, `/superdocs:review_contract_for_redflags` |
...
You should see `superdocs` in the list. Then in a Claude Code session, type `/` and you'll see the 5 workflow prompts in the slash menu.
...
The plugin itself is small — just the manifest + the skill markdown. The 21 tools and 5 prompts are served live from `https://api.superdocs.app/mcp/` (Streamable HTTP, sk_ Bearer auth). Tool descriptions, prompt templates, and behavior changes ship from the backend instantly — no plugin update needed for those.
