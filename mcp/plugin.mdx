...
|---|---|---|
| **MCP tools** | 35 | Chat, structural editing, attachments, sessions, multi-document, durable Files, cross-session memory, revert, jobs, templates, pre-signed upload/download |
...
The plugin itself is small — just the manifest + the skill markdown. The 35 tools and 4 prompts are served live from `https://api.superdocs.app/mcp/` (Streamable HTTP, sk_ Bearer auth). Tool descriptions, prompt templates, and behavior changes ship from the backend instantly — no plugin update needed for those.
